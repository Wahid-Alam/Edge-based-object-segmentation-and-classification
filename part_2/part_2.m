...
gray_edge = edge(gray_im,'canny',0.2,10);   
...
len = max(size(I)); % Uses image size to scale our thresholds for circle radii 

min_radius_range = round(len/80); 

max_radius_range = round(len/12); 

[all_centers,all_radii] = imfindcircles(G,[min_radius_range max_radius_range],'Method','TwoStage', 'EdgeThreshold',0.2, 'Sensitivity', 0.9); % Inner and outer circles  

largest_radius = max(all_radii); 

radius_thresh = largest_radius/1.43; 

  

[pre_centers_i,pre_radii_i] = imfindcircles(G,[min_radius_range ceil(radius_thresh)],'Method','TwoStage', 'EdgeThreshold',0.2, 'Sensitivity', 0.9); % Inner circles  

[centers_o,radii_o] = imfindcircles(G,[ceil(radius_thresh)+1 max_radius_range],'Method','TwoStage', 'EdgeThreshold',0.2, 'Sensitivity', 0.9); % Outer circles  
...
if exist('centers_i','var') && exist('centers_i','var') 

    viscircles(centers_i,radii_i ,'EdgeColor','r')    % Detected cirlce overlay etected cirlce overlay  

    hold on 

    viscircles(centers_o,radii_o ,'EdgeColor','b')    % Detected cirlce overlay etected cirlce overlay  

else 

    viscircles(center,radius ,'EdgeColor','b')    % Detected cirlce overlay etected cirlce overlay   
...
figure(8); imshow(seg,[]); impixelinfo  
...
figure(9);imshow(I1, []); xlabel('Detected Image')   
toc